...
function App() {
    ...
    // Report the final score to the Watson Games portal when a run ends.
    const wasStarted = useRef(false);
    useEffect(() => {
        if (wasStarted.current && !gameStarted) {
            const score = stack.length - 1;
            if (score > 0 && window.WatsonGames) {
                window.WatsonGames.reportScore(score, { final: true });
            }
        }
        wasStarted.current = gameStarted;
